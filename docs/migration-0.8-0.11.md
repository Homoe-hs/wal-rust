...
> 🟡 **历史区间指南(0.8 → 0.11)** —— 面向从上一代 WAL 迁移的用户;0.11 之后的变更见 `CHANGELOG.md`。
...
| 项 | 行为 |
|---|---|
| `count`/`find` **默认** | **逐索引(官方语义)**: 从头扫到尾, 由统一区间扫描引擎实现(复杂度 O(变更点×表达式), 不是逐拍);`count/step`/`find/step` 是逐拍等价形式(慢, 主要用于对拍与调试) |
