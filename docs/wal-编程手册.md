...
> **字面量口径**(0.14.18 起): 整数 `42` / `-7`、浮点 `3.14`、**科学计数法 `1e3` `1.5e3` `1.5E-3`**、
> 二进制 `0b1011`、十六进制 `0xFF`、布尔 `#t` / `#f` / `true` / `false` / `nil`、字符串 `"..."`。
> `#name` 是**分组符号**(见 §4.1 组), 不是注释 —— 例如 `#timeout` 表示"当前组里的 timeout 信号",
> 找不到组/信号时给的是分组解析错误(而不是把 `#t` 和 `imeout` 拆开)。
> 索引类参数(如 `(sample-at s idx)`)必须传整数;需要整数除法用 `(div a b)`。
