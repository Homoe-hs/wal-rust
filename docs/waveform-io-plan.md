...
> 🟡 **计划文档(部分已落地)** —— 调研与阶段计划;每阶段的落地状态写在 §3 的表里, 不要直接当待办清单。

> 关联: `docs/query-engine-design.md`(查询侧统一引擎)。
> 目标: 150GB VCD 加载 ≤ 2-3 分钟(基线为 0.11.11: 58.7GB 加载 360s、外推 150GB ≈ 900s;0.13.2 起 `load` 只读文件头 ≈ 1.1s,见 README「性能」),
> 单信号查询 ≤ 60s,解析吞吐 ≥ 2GB/s(0.11.11 基线: 有效 0.5-0.6GB/s;0.13.2 已接近单遍读上限,见 README「性能」),RSS ≤ 3GB。
...
> 状态(2026-09 复核): 这是**计划文档**, 其中 IO-2/IO-3 的主体已在 0.12.x–0.13.x 落地
> (稀疏锚点 + 旁挂列缓存 + 两段式懒索引, 见 `AGENTS.md` 的"性能敏感路径"与 README「性能」);
> IO-1/IO-5 的行分类与预筛已按需实现, 但**没有** SIMD 化与 zone map 这两件事。
> 逐行状态见下表 —— 不要把本表当成待办清单直接开工。

| 阶段 | 内容 | 验收 | 状态 |
|---|---|---|---|
| IO-1 | PASS-1b 行边界 SIMD 化(只改行分裂,不解值);值行 id 尾匹配查表 | 解析吞吐 ≥1GB/s;门禁 ALL MATCH | 🟡 部分(`memchr` 行分裂 + 廉价预筛已做;无 SIMD) |
| IO-2 | 稀疏索引 BTreeMap → 块局部表 + 归并(M 保留现有读取逻辑,只换构建) | 加载时间 -40~60%;行为不变 | ✅ 已落地(稀疏锚点 `Vec<(ts,off)>` + 并行分块) |
| IO-3 | 值紧凑打包(delta 索引 + 2bit x/z) + 列缓存替换 LRU 重扫 | get 同信号二次查询 <0.1s;RSS 下降 | ✅ 主体已落地(`signal_cache` + 旁挂 `.cols` 缓存) |
| IO-4 | 分块 madvise 细粒度化 | RSS ≤3GB @150GB | ✅ 已落地(64MB 窗口 WILLNEED/DONTNEED) |
| IO-5 | zone map 剪块 + predicate 早退(is-x 列状态集) | 稀疏信号查询再 3-10x | ❌ 未做 |
| IO-6 | 与查询引擎(P1-P6)合流:Column 直喂 IntervalSweep | 单套 IO 路径 | ⏳ 部分(引擎已消费变更列;`Column` 抽象未落地) |
...
## 5. 追加调研:冷查询(磁盘重读)的治本方案(2026-09-07)
...
- 位置: 默认 **执行命令的当前目录** `./.wal-rust-cache/`(相对 CWD,可用 `WAL_CACHE_DIR` 覆盖)。
  **绝不写在波形所在目录**——波形可能来自只读挂载/共享盘;缓存目录不可写时静默跳过,
  查询结果不受影响。
- 命名: `<basename>-<len>-<mtime>-<ctime>-<inode>-v1.wcol`(0.14.8 起 key 含 ctime/inode, 见 `src/trace/vcd.rs::file_identity`)。
- 内容(顺序读一次即得,无需解析):
...
- 失效: (**size, mtime, ctime, inode**) 任一变化、首尾 64KB 指纹变化, 或格式版本不符 → 视为未命中并重建。
  (0.14.8 补 ctime/inode: 只按 size+mtime 会漏掉"同一秒内等长改写", 见 `docs/fsdb-npi.md` §9.3)
- 开关: `WAL_CACHE=off|auto|build|read`(默认 auto: 命中读;**未命中且波形 ≥8MB 时写回**,
  小文件不写以免污染目录;`build` 显式构建,始终写;`read` 只读不建)。
- 目录污染防护(0.12.28): auto 模式设 8MB 阈值(`WAL_CACHE_MIN_MB` 可调);缓存目录不可写时静默跳过(查询结果不受影响)。

**内存计量口径(B7)**: 波形用 mmap 读取,RSS 包含**文件页驻留**(内核把映射页算进 RSS),
因此"17.5GB 波形 → RSS 27–32GB"属于正常现象;进程**堆内存**是 O(信号数 + 被查询信号的变更列),
远小于波形体积。要看真实占用请用 `smaps_rollup` 的 Private_Dirty,或对比 `/proc/<pid>/smaps` 的
file-backed 部分。加载结束时会对映射做一次 `madvise(DONTNEED)`,页会在后续扫描时按需重新调入。

**旁挂列缓存(0.12.36)**: `.wcol` 只存稀疏锚点,跨进程仍要为每个"新信号"重扫全文件。
现在冷扫描/列构建完成后,把该信号的**已解码变更列**按信号落盘:

- 位置: 与 `.wcol` 同 key 的 `<wave>-<len>-<mtime>-v1.cols/` 目录,每信号一个 `<fnv(全名)>.col`;
- 写入: `WAL_CACHE=auto|build` 且波形 ≥ `WAL_CACHE_MIN_MB`(默认 8MB);只缓存**实际查询过**的信号;
- 读取: `anchored_changes` 最优先查旁挂列(先于内存列/锚定扫描)→ 命中即免扫全文件;
- 失效: 目录名含 size+mtime,与 `.wcol` 同步;
- 实测 58.7GB: 同一信号同一查询,**第二个进程 113.8s → 3.35s**(RSS 3.3GB → 1.2GB;
  3.35s 里几乎全是 `.wcol` 索引加载,查询本身毫秒级)。
