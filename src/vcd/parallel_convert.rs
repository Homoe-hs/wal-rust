...
// ---- standalone 0x08 block builder (extracted from writer.rs) ----
...
fn build_08_block(
    ...
    while h <= idx_limit {
        ...
        idx_buf.extend_from_slice(&crate::fst::varint::encode_fst_svarint(sval));
        h += 1;
    }
...
// ---- VCD byte-level parsing helpers ----
...
fn parse_vcd_header(data: &[u8]) -> Result<(
    HashMap<Vec<u8>, u32>,
    Vec<u32>,
    usize,
    u32,
...
fn find_hash_positions(data: &[u8], chunk_end: usize) -> Vec<usize> {
    let mut positions = Vec::new();
    let mut pos = 0;
    while pos < chunk_end {
        if data[pos] == b'\n' || pos == 0 {
            let line_start = if pos == 0 { 0 } else { pos + 1 };
            if line_start < chunk_end && data[line_start] == b'#' {
                positions.push(line_start);
            }
        } else if data[pos] == b'#' && (pos == 0 || data[pos - 1] == b'\n') {
            // Also check: first char after prev newline
        }
        pos += 1;
    }
    // Also handle the first byte being #
    if positions.is_empty() && data[0] == b'#' {
        positions.push(0);
    }
    positions
}
...
fn process_chunk(
    ...
    while pos < chunk_len {
        let line_start = pos;
        let line_end = match chunk_data[pos..].iter().position(|&b| b == b'\n') {
            Some(nl) => { pos = line_start + nl + 1; line_start + nl }
            None => { pos = chunk_len; chunk_len }
        };

        let pct = ((chunk_offset + line_start as u64) as f64 / total_len as f64 * 100.0).round() as u8;
        if pct > last_progress_pct && pct % 5 == 0 {
            progress.fetch_add(pct as u64 - last_progress_pct as u64, Ordering::Relaxed);
        ...
        if first == b'#' {
            ...
            if let Some((id, val)) = parse_value_change(line) {
                if let Some(&handle) = signal_map.get(&id) {
                    let time_idx = (timestamps.len().saturating_sub(1)) as u32;
                    entries.push(BlockEntry { handle, time_idx, value: val });
                }
            }
        }
    }
    ...
    let block = build_08_block(&timestamps, &entries, signal_widths, max_handle, begin_time, end_time);
...
pub fn vcd_to_fst_parallel(
    ...
    let data = crate::vcd::convert::mmap_or_read(vcd_path)?;
    let file_len = data.len();

    let (signal_map, signal_widths, dump_start, handle_count) = parse_vcd_header(&data)?;
    let dump_data = &data[dump_start..];

    // Find # positions in dump for chunking
    let hash_pos = find_hash_positions(dump_data, dump_data.len());
    if hash_pos.is_empty() {
        return Err("No timestamps found in VCD dump section".to_string());
    }

    // Split into chunks (number of threads)
    let num_threads = std::cmp::max(1, num_cpus::get().min(hash_pos.len()));
    let mut chunk_boundaries: Vec<usize> = vec![0; num_threads + 1];
    chunk_boundaries[num_threads] = dump_data.len();
    let hashes_per_chunk = hash_pos.len() / num_threads;
    for i in 1..num_threads {
        let idx = i * hashes_per_chunk;
        chunk_boundaries[i] = hash_pos[idx.min(hash_pos.len() - 1)];
    }
    ...
    let results: Vec<Result<(Vec<u8>, u64), String>> = {
        use rayon::prelude::*;
        (0..num_threads).into_par_iter().map(|i| {
            let start = chunk_boundaries[i];
            let end = chunk_boundaries[i + 1];
            let chunk = &dump_data[start..end];
            process_chunk(chunk, dump_start as u64 + start as u64, file_len as u64, &signal_map, &signal_widths, handle_count, progress)
        }).collect()
    };

    // Write FST
    let mut blocks: Vec<Vec<u8>> = Vec::with_capacity(num_threads);
    let mut total_timestamps: u64 = 0;
    for r in results {
        let (block, ts_count) = r?;
        if !block.is_empty() {
            total_timestamps += ts_count;
            blocks.push(block);
        }
    }

    // Create FstWriter to set up signals, write HDR
    let fst_file = fs::File::create(fst_path)
        .map_err(|e| format!("Failed to create {}: {}", fst_path.display(), e))?;
    // We use FstWriter just for HDR, GEOM, HIER — write 0x08 blocks directly
    ...
    writer.set_timescale(-12);

    // Register signals in order of creation
    // Parse header again to get signals in proper order
    let _ = parse_vcd_header_and_register(&data, &mut writer)?;
...
fn parse_vcd_header_and_register(data: &[u8], writer: &mut FstWriter<impl std::io::Write>) -> Result<(), String> {
    let mut pos: usize = 0;
    let file_len = data.len();
    let mut in_directive = false;
            ...
            if let Some(name_end) = line.windows(4).position(|w| w == b"$end") {
                ...
                if let Some(second_space) = line[name_start + 1..].iter().position(|&b| b == b' ') {
                    let sname_bytes = &line[name_start + 1 + second_space + 1..name_end];
                        let trimmed = sname_bytes.trim_ascii_end();
                        if let Ok(sname) = std::str::from_utf8(trimmed) {
                            if !sname.is_empty() {
                            writer.push_scope(sname, ScopeType::VcdModule);
                            scope_stack.push(sname.to_string());
                        }
                    }
                }
            } else { in_directive = true; }
        } else if line.starts_with(b"$var") {
            use crate::fst::VarType;
            let parts: Vec<&[u8]> = line.split(|&b| b == b' ' || b == b'\t').filter(|p| !p.is_empty()).collect();
            if parts.len() >= 5 && parts.last().map(|p| *p == b"$end" || p.ends_with(b"$end")).unwrap_or(false) {
                if let Ok(width) = std::str::from_utf8(parts[2]).unwrap_or("0").parse::<u32>() {
