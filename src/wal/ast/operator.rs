...
pub enum Operator {
    ...
    Mul,
    Div,
    /// 整数除法(向零取整): `(div 10 3)` = 3(内测反馈: 硬件语境常要整除)
    DivInt,
    Exp,
    Floor,
    Ceil,
    ...
    SignalWidth,
    SampleAt,
    /// (initial "sig") → $dumpvars 快照(t0 写入的原值; 没有初值条目则为 x)
    Initial,
    TrimTrace,
    Count,
    Timeframe,
    ...
    // Waveform time queries (time-aware, unlike index-based find/count)
    GetWave,
    Wave,
    At,
    CountRise,
...
impl Operator {
    /// All operator names, for error suggestions and (help).
    ...
    pub fn from_str(s: &str) -> Option<Self> {
        match s {
            ...
            "boolean?" => Some(Operator::IsBool),
            "bool?" => Some(Operator::IsBool),
            "for/list" => Some(Operator::ForList),
            "count/step" => Some(Operator::CountStep),
            "find/step" => Some(Operator::FindStep),
            "null?" => Some(Operator::IsNull),
            "empty?" => Some(Operator::IsNull),
            "take" => Some(Operator::Take),
            ...
            "initial" | "t0" => Some(Operator::Initial),
            ...
            // Waveform time queries
            "getwave" => Some(Operator::GetWave),
            "wave" => Some(Operator::Wave),
            "at" => Some(Operator::At),
            "count-rise" => Some(Operator::CountRise),
            "count-fall" => Some(Operator::CountFall),
            "count-edges" => Some(Operator::CountEdges),
            "edges" => Some(Operator::Edges),
            "find-sig" => Some(Operator::FindSig),
            "search" => Some(Operator::Search),
            "assert-eq" => Some(Operator::AssertEq),
            "period" => Some(Operator::Period),
...
// Virtual
    ...
    pub fn as_str(&self) -> &'static str {
        match self {
            ...
            Operator::IsBool => "boolean?",
            Operator::ForList => "for/list",
            Operator::CountStep => "count/step",
            Operator::FindStep => "find/step",
            ...
            Operator::GetWave => "getwave",
            Operator::Wave => "wave",
            Operator::At => "at",
            Operator::CountRise => "count-rise",
            Operator::CountFall => "count-fall",
            Operator::CountEdges => "count-edges",
            Operator::Edges => "edges",
            Operator::FindSig => "find-sig",
            Operator::Search => "search",
            Operator::AssertEq => "assert-eq",
